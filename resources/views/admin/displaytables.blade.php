...
<table >
	...
	<tr class={{($t->exists)? "greenrow":"redrow"}}>
		...
		<td><a href={{"/admin/" . $t->tableSafeName}} >{{$t->tableSafeName}}</a></td> <!-- NAME --></td>
		...
		@if($t->manager() && $t->manager()->updated_at !== null)
		<td>{{$t->manager() && $t->manager()->updated_at->diffForHumans() }}</td>
		@endif
		@if($t->manager() && $t->manager()->created_at !== null)
