...
class Viewer {
    ...
    public function loadTitles($args){
      ...
      if(isset($this->attributes['dbfs']['inventory'])){        
        $list = [];
        $parameters = Misc::defaultParameters($args);
        $count = 0;
        ...
        foreach($this->dbfs['inventory']->records AS $book){
          ...
          if(Compare::test($book, $parameters)){
            $count++;
            ...
            $list[] = new \App\Models\Inventory($book);
    ...
    private function initArgs($args)
    {
        ...
        if(isset($this->args['preload'])){
            ...
            $dbfs = [
                "inventory" => [Inventory::class,"inventory",["CAT","AUTHORKEY","PUBDATE","INVNATURE"]]
            ];
    ...
    private function calcBrowseProducts(){
        ...
        foreach($cats AS $cat){
            $genre_items[] = Misc::makeSearchUrl($cat, "CAT");
        }
        
        $months = [
          Misc::makeSearchUrl("202003", "PUBDATE", "2020 March"),
          Misc::makeSearchUrl("202004", "PUBDATE", "2020 April"),
          Misc::makeSearchUrl("202005", "PUBDATE", "2020 May")
        ];
